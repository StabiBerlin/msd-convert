<?php
	...
	// $raw = file_get_contents('https://slavistik-portal.de/msd/raw/SchmSow_1689077832_Page_101-200 _Ne.zip');
	...
	foreach($files as $file) {
		...
		$html = preg_replace("@<span style=\'text-transform:uppercase\'>\s<\/span>@mis",' ',$html);
		$html = preg_replace("@<span style=\'text-transform:uppercase\'><\/span>@mis",' ',$html);
		$html = preg_replace("@\"Breitkopf Fraktur\".*?\'>@mis",'"Breitkopf Fraktur"\'>',$html);
		$html = preg_replace("@<i>(.*?)<\/i>@mis",'<i>{$1}</i>',$html);
		$html = preg_replace("@\"Breitkopf Fraktur\"\'>(.*?)<\/span@mis",'\"Breitkopf Fraktur\"\'>#m1#$1#m2#</span',$html);
	...
	for ($i = 0; $i < count($pages[0]); $i++) {
		...
		$pages[0][$i] = preg_replace('@#m1#@mis', '[fra=', $pages[0][$i]);		
		$pages[0][$i] = preg_replace('@#m2#@mis', ']', $pages[0][$i]);		
		$pages[0][$i] = preg_replace('@\s]@mis', '] ', $pages[0][$i]);
		$pages[0][$i] = preg_replace('/\040{1,}/m',' ',$pages[0][$i]);			
		$pages[0][$i] = preg_replace('@\]\s\[fra=@mis', ' ', $pages[0][$i]);		
		$pages[0][$i] = preg_replace('@\.\[@mis', '. [', $pages[0][$i]);		
		$pages[0][$i] = preg_replace('@\]\.@mis', ']', $pages[0][$i]);		
		$pages[0][$i] = preg_replace('@\]-@mis', '-]', $pages[0][$i]);		
		$pages[0][$i] = preg_replace('@\]\s-\[fra=@mis', ' -', $pages[0][$i]);		
		...
		// Kosmetik	
		$pages[0][$i] = preg_replace('/\[fra=\]/mis',"",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\[fra=ъ\]/mis',"ъ",$pages[0][$i]);


		// Grammatische Markierungen
		$pages[0][$i] = preg_replace('@\.\s\}@m',".} ",$pages[0][$i]); 
		$pages[0][$i] = preg_replace('@\}\[@m',"} [",$pages[0][$i]); 
		$pages[0][$i] = preg_replace('@\,\s\]@m',"], ",$pages[0][$i]); 
		$pages[0][$i] = preg_replace('@\}\.@m',".}",$pages[0][$i]); 
		$pages[0][$i] = preg_replace('@(\})([a-z])@m',"$1 $2",$pages[0][$i]); 
		...
		$pages[0][$i] = preg_replace('/\s\[fra=\.\]/mis',"",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\[fra=\.\]/mis',"",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\[fra=,\s/mis'," [fra=",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\{,\s/mis',", {",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\{\s/mis',"{",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\s}/mis',"} ",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\{\}/mis'," ",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\?/mis',"? ",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\}\s\[fra=\./mis',".} [fra=",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\[fra=\s/mis',"[fra=",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\.\]/mis',"].",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/ —}/mis',"} —",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/—}/mis',"} —",$pages[0][$i]);
		
		$pages[0][$i] = preg_replace('/\{s\}/mis',"{s.}",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\{m\}/mis',"{m.}",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\{f\}/mis',"{f.}",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/\{w\}\s\{/mis',"{w ",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/(\})([а-яa-z])/mis',"$1 $2",$pages[0][$i]);
		$pages[0][$i] = preg_replace('/z\[/mis',"[",$pages[0][$i]);
		
			$pages[0][$i] = preg_replace('/\sv\.\s/mis'," {v.} ",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\sm\.\s/mis'," {m.} ",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\sn\.\s/mis'," {n.} ",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\sf\.\s/mis'," {f.} ",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\ss\.\s/ms'," {s.} ",$pages[0][$i]);
			#$pages[0][$i] = preg_replace('/\snd\.\s/ms'," {nd.} ",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\spl\.\s/ms'," {pl.} ",$pages[0][$i]);

			$pages[0][$i] = preg_replace('/\sm\.$/mis'," {m.}",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\sn\.$/mis'," {n.}",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\sf\.$/mis'," {f.}",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\ss\.$/ms'," {s.}",$pages[0][$i]);
			#$pages[0][$i] = preg_replace('/\snd\.$/ms'," {nd.}",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\spl\.$/ms'," {pl.}",$pages[0][$i]);

			$pages[0][$i] = preg_replace('/\sm\.([,;])/mis'," {m.}$1",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\sn\.([,;])/mis'," {n.}$1",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\sf\.([,;])/mis'," {f.}$1",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\ss\.([,;])/ms'," {s.}$1",$pages[0][$i]);
			#$pages[0][$i] = preg_replace('/\snd\.([,;])/ms'," {nd.}$1",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/\spl\.([,;])/ms'," {pl.}$1",$pages[0][$i]);
			
			$pages[0][$i] = preg_replace('/\{s\.}\sd\./ms',"{s. d.}",$pages[0][$i]);
			#$pages[0][$i] = preg_replace('/s\.\}\s\{nd\./ms',"s. nd.",$pages[0][$i]);
			$pages[0][$i] = preg_replace('/s\.\}\s\{d\./ms',"s. d.",$pages[0][$i]);
			
			$pages[0][$i] = preg_replace('/\}\s\{/ms'," ",$pages[0][$i]);
		
		 // Verweise zurück
		$pages[0][$i] = preg_replace('@\sv\.\}@m',"} {v.}",$pages[0][$i]); 
	...
	function getwords($value){
		...
		$value = preg_replace('/[\:\.,;\(\)\{\}\[\]]/m'," ",$value);
		$value = preg_replace('/fra=/m',"",$value);
		$value = preg_replace('/\#/m',"",$value);
		$value = preg_replace('/\!/m',"",$value);
